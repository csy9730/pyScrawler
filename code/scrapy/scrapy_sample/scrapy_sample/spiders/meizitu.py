...
class MeizituSpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        urls = response.xpath('.//*[@id="wp_page_numbers"]/ul/li/@href').getall()
        for url in urls:
            print(url)
            yield  Request(url ,headers=self.header,callback=self.parse_page)
    def parse_page(self, response):
        urls = response.xpath('li//h3[@class="tit"]/@href').getall()
        for url in urls:
            print(url)
