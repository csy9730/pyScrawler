...
class DuduSpider(scrapy.Spider):
    ...
    #起始url，列表内字符串的拼接    
    def __init__(self,start_params=[{"word":"moon","pn":0,"pg_range":1}],**kwargs):
        super(DuduSpider, self).__init__( **kwargs)
        self.start_urls =[self.base_url % ( dct["word"],str( ( dct["pn"]+i)*30 )) for dct in start_params  for i in range(int(dct["pg_range"])) ]
    def parse(self, response):        
        #使用scrapy shell 返回的内容为json格式，正则匹配出图片链接并提取
        pattern = re.compile(r'"middleURL":"(.*?)",', re.S)
        datas = re.findall(pattern, response.text)
        word = re.search(r"word=(\w+)&",response.url)[1]
        img_folder =  word+'/'        
        item = ImageItem()
